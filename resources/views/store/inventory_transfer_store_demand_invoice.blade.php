...
<?php $new_format_demand_id = 1215; ?>
<?php $store_data = json_decode($demand_data->store_data,false); ?>
<?php $from_store_data = json_decode($demand_data->from_store_data,false); ?>
    ...
    <table class="table-1">
        ...
        <tr>
            <td width="50%" valign="top" >
                <b class="text-1"> @if($gst_name != '') Invoice To @else Challan To @endif</b><br/>
            ...
            <td width="50%" valign="top" align="right" class="text-1">
                <b>@if($gst_name != '') Invoice From @else Challan From @endif</b><br/>
                <b>{{strtoupper($from_store_data->gst_name)}}</b><br/>
                <span class="text-2">Address : {{$from_store_data->address_line1}}<br/>{{$from_store_data->address_line2}}<br/>{{$from_store_data->city_name}} - {{$from_store_data->postal_code}}<br/>
                Ph.No. : {{$from_store_data->phone_no}}<br/>
                GSTIN NO.: {{$from_store_data->gst_no}}</span>
            </td>
        </tr>
    </table>
...
@if($pdf_type == 'debit_credit_note')
    @if(strtolower($debit_credit_note_type) == 'debit_note')
        <table class="table-1">
            @if($gst_name == '')
                <tr><td></td><td class="heading-1">Delivery Challan {{$cancel_str}}</td></tr>
            @else
    ...
    @if(strtolower($debit_credit_note_type) == 'credit_note')
        <table class="table-1">
            @if($gst_name == '')
                <tr><td></td><td class="heading-1">Delivery Challan {{$cancel_str}}</td></tr>
            @else
    ...
    @if(strtolower($debit_credit_note_type) == 'debit_note')
        <table class="table-1 border-top border-bottom" >
            <tr><td width="50%">Debit Note No:&nbsp;{{$demand_data->invoice_no}}<span class="text-3"> </span></td><td width="50%">Debit Note Date:&nbsp;<span class="text-3">{{date('d/m/Y H:i',strtotime($demand_data->created_at))}} </span></td></tr>
        </table>    
    @endif
    
    @if(strtolower($debit_credit_note_type) == 'credit_note')
        <table class="table-1 border-top border-bottom" >
            <tr><td width="50%">Credit Note No:&nbsp;{{$demand_data->credit_invoice_no}}<span class="text-3"> </span></td><td width="50%">Credit Note Date:&nbsp;<span class="text-3">{{date('d/m/Y H:i',strtotime($demand_data->credit_note_date))}} </span></td></tr>
        </table>    
    @endif
@endif
