...
<?php $new_gst_round_demand_id = 2476; ?>
    ...
    @if(!empty($push_demand_list))
        <table class="table-1  border-bottom" >
            <tr><td width="100%"><span class="text-2">Invoice Nos: {{implode(', ',$push_demand_list)}}</span></td></tr>
        </table> 
    @endif
    ...
    <table class="table-1" >
        ...
        @foreach($demand_sku_list as $sku=>$product_data)    
            <tr>
                ...
                <td>{{($product_data['prod']->arnon_inventory == 0)?$product_data['prod']->vendor_sku:$product_data['prod']->product_sku}}</td>
                @if($demand_data->invoice_type  == 'product_id')
