@extends('layouts.default')
@section('content')

<?php echo CommonHelper::displayPageErrorMsg($error_message); ?>

@if(empty($error_message))
    <?php $breadCrumbArray = array(array('name'=>'Dashboard','link'=>'dashboard'),array('name'=>'Purchase Orders')); ?>
    <?php echo CommonHelper::pageSubHeader($breadCrumbArray,'Purchase Orders'); ?>

    <section class="product_area">
        <div class="container-fluid" >
            <form method="get">
                
                <div class="row justify-content-end">
                    <div class="col-md-1">
                        <input type="text" name="po_id" id="po_id" class="form-control" value="{{request('po_id')}}" placeholder="PO ID">
                    </div>
                    <div class="col-md-2">
                        <input type="text" name="po_no" id="po_no" class="form-control" value="{{request('po_no')}}" placeholder="PO No">
                    </div>
                    <div class="col-md-2">
                        <input type="text" name="invoice_no" id="invoice_no" class="form-control" value="{{request('invoice_no')}}" placeholder="PO Invoice No">
                    </div>
                    <div class="col-md-2">
                        <select name="po_cat_id" id="po_cat_id" class="form-control">
                            <option value="">-- PO Category --</option>
                            @for($i=0;$i<count($po_category_list);$i++)
                                <?php if($po_category_list[$i]['id'] == request('po_cat_id')) $sel = 'selected';else $sel = ''; ?>
                                <option {{$sel}} value="{{$po_category_list[$i]['id']}}">{{$po_category_list[$i]['name']}}</option>
                            @endfor   
                        </select>
                    </div>
                    <div class="col-md-2">
                        <select name="v_id" id="v_id" class="form-control">
                            <option value="">-- Vendor --</option>
                            @for($i=0;$i<count($vendors_list);$i++)
                                <?php if($vendors_list[$i]['id'] == request('v_id')) $sel = 'selected';else $sel = ''; ?>
                                <option {{$sel}} value="{{$vendors_list[$i]['id']}}">{{$vendors_list[$i]['name']}}</option>
                            @endfor   
                        </select>
                    </div>
                    
                    <div class="col-md-1"><input type="submit" name="search" id="search" value="Search" class="btn btn-dialog" ></div>
                    <div class="col-md-1"><?php echo CommonHelper::displayDownloadDialogButton('Purchase Orders'); ?></div>
                </div>    
                <div class="row justify-content-end" style="margin-top: 10px;">
                    @if(in_array($user->user_type,[1,3]) || $is_fake_inventory_user)
                        <div class="col-md-2" ><a  class="btn btn-dialog" href="{{url('purchase-order/product/create')}}">Create PO</a> </div>
                    @endif
                    
                    @if(in_array($user->user_type,[1,3]))
                        <div class="col-md-4" ><a  class="btn btn-dialog" href="{{url('purchase-order/product/static/create')}}">Create Static PO</a> 
                        <a  class="btn btn-dialog" href="{{url('purchase-order/bulk/finished/create')}}">Create Stitching PO</a></div>
                    @endif 
                </div>
            </form>
            <div class="separator-10"></div>
            <div id="purchaseOrdersErrorMessage" class="alert alert-danger elem-hidden"></div>
            <div id="productsContainer">
                <div class="table-responsive table-filter">
                    <table class="table table-striped admin-table" cellspacing="0">
                        <thead><tr class="header-tr"><th>Order ID</th><th>Order No</th><th>Vendor</th><th>Category</th><th>Delivery Date</th><th>Invoice Count</th><th>Created By</th><th>Created On</th><th>Details</th></tr></thead>
                        <tbody>
                            @for($i=0;$i<count($purchase_orders);$i++)
                                <tr>
                                    <td>{{$purchase_orders[$i]->id}}</td>
                                    <td>{{$purchase_orders[$i]->order_no}}</td>
                                    <td>{{$purchase_orders[$i]->vendor_name}}</td>
                                    <td>{{$purchase_orders[$i]->po_category_name}}</td>
                                    <td>@if(!empty($purchase_orders[$i]->delivery_date)) {{date('d M Y',strtotime($purchase_orders[$i]->delivery_date))}} @endif</td>
                                    <td>{{$purchase_orders[$i]->invoice_count}}</td>
                                    <td>{{$purchase_orders[$i]->user_name}}</td>
                                    <td>{{date('d M Y, H:i',strtotime($purchase_orders[$i]->created_at))}}</td>
                                    <td>
                                        <a href="{{url('purchase-order/product/detail/'.$purchase_orders[$i]->id)}}"><i title="Order Details" class="fas fa-eye"></i></a> &nbsp; 
                                        
                                        @if($user->user_type == 6 || $user->user_type == 3 || $user->user_type == 1 || $is_fake_inventory_user)
                                            <a href="{{url('purchase-order/product/invoice/list/'.$purchase_orders[$i]->id)}}"><i title="PO Invoices" class="fas fa-eye"></i></a> &nbsp;   
                                        @endif        
                                        
                                        @if($user->user_type == 3 || $user->user_type == 1)
                                            @if($purchase_orders[$i]->static_po == 1)
                                                <a href="{{url('purchase-order/product/static/edit/'.$purchase_orders[$i]->id)}}"><i title="Edit Purchase Order Items" class="fas fa-edit"></i> </a> 
                                            @else
                                                <a href="{{url('purchase-order/product/edit/'.$purchase_orders[$i]->id)}}"><i title="Edit Purchase Order Items" class="fas fa-edit"></i> </a> 
                                            @endif
                                            &nbsp; <a href="{{url('purchase-order/edit/data/'.$purchase_orders[$i]->id)}}"><i title="Edit Purchase Order Data" class="fas fa-edit"></i> </a> 
                                        @endif    
                                    </td>
                                </tr>
                            @endfor
                        </tbody>
                    </table>
                    {{ $purchase_orders->withQueryString()->links() }}
                        <p>Displaying {{$purchase_orders->count()}} of {{ $purchase_orders->total() }} orders.</p>
                </div>
            </div>
        </div>
    </section>

    <?php echo CommonHelper::displayDownloadDialogHtml($purchase_orders->total(),1000,'/purchase-order/product/list','Download Purchase Orders List','Purchase Orders'); ?>

@endif

@endsection

@section('scripts')

@endsection
