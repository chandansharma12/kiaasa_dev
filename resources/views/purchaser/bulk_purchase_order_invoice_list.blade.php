...
@if(empty($error_message))
    <?php $breadCrumbArray = array(array('name'=>'Dashboard','link'=>'dashboard'),array('name'=>'Purchase Orders','link'=>'purchase-order/bulk/list'),array('name'=>'Purchase Order Invoice list')); ?>
            ...
            <form method="get">
                @if($user->user_type == 6)
                    <div class="row justify-content-end" >
                        <div class="col-md-2" ><button type="button" id="pos_add_invoice_submit" name="pos_add_invoice_submit" class="btn btn-dialog" value="Submit" onclick="displayAddBulkPOInvoice();">Add Invoice</button>&nbsp;&nbsp;</div>
                    </div>
                @endif
            </form>
                        ...
                        <tbody>
                            @for($i=0;$i<count($po_invoices);$i++)
                                <tr>
                                    ...
                                    <td>
                                        <a href="javascript:;" onclick="displayBulkPOInvoiceDetails({{$po_invoices[$i]->id}});"><i title="Invoice Details" class="fas fa-eye"></i> &nbsp; </a>
                                        @if($user->user_type == 6 || !empty($po_invoices[$i]->grn_id))    
                                            <a href="{{url('purchase-order/bulk/items/import/'.$po_invoices[$i]->id)}}"><i title="Import Inventory" class="fas fa-eye"></i> &nbsp;  </a>
                                        @endif    
                                        
                                        @if($user->user_type == 6 || !empty($po_invoices[$i]->qc_id))    
                                            <a href="{{url('purchase-order/bulk/items/qc/'.$po_invoices[$i]->id)}}"><i title="Inventory QC" class="fas fa-eye"></i>     </a>
                                        @endif    
                                    </td>
                                </tr>
                            @endfor
            ...
            <div class="modal-content">
                ...
                <form class="" name="addVehicleDetailsForm" id="addVehicleDetailsForm" type="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-2">
                                ...
                                <input id="invoice_no" type="text" class="form-control" name="invoice_no" value="" autocomplete="off">
                                <div class="invalid-feedback" id="error_validation_invoice_no"></div>
                            </div>
                            <div class="form-group col-md-2">
                                <label>Invoice Date</label>
                                <input id="invoice_date" type="text" class="form-control" name="invoice_date" value="" autocomplete="off">
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <input id="vehicle_no" type="text" class="form-control" name="vehicle_no" value="" autocomplete="off">
                    ...
                    <div class="modal-footer center-footer">
                        ...
                        <button type="button" id ="add_vehicle_details_confirm_submit" name="add_vehicle_details_confirm_submit" class="btn btn-dialog" onclick="submitAddBulkPOInvoice();">Submit</button>
                        <input type="hidden" name="po_id" id="po_id" value="{{$po_data->id}}">
                    </div>
                </form>
            </div>
                ...
                <form class="" name="vehicleDetailForm" id="vehicleDetailForm" type="POST" >
                    ...
                    @if($user->user_type == 6)
                        <div class="modal-footer center-footer">
                            ...
                            <button type="button" id ="update_po_invoice_detail_submit" name="update_po_invoice_detail_submit" class="btn btn-dialog" onclick="submitUpdateBulkPOInvoiceDetails();">Submit</button>
