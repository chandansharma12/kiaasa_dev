...
@if(empty($error_message))
    <?php $breadCrumbArray = array(array('name'=>'Dashboard','link'=>'dashboard'),array('name'=>'Fabric Purchase Orders')); ?>
    <?php echo CommonHelper::pageSubHeader($breadCrumbArray,'Fabric Purchase Orders'); ?>
            ...
            <form method="get">
                ...
                <div class="row justify-content-end">
                    ...
                    @if(in_array($user->user_type,[1,3]))
                        <div class="col-md-2" ><a  class="btn btn-dialog" href="{{url('purchase-order/bulk/create')}}">Create Order</a></div>
                    @endif
                </div>
            </form>
                        ...
                        <tbody>
                            @for($i=0;$i<count($purchase_orders);$i++)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{url('purchase-order/bulk/detail/'.$purchase_orders[$i]->id)}}"><i title="Order Details" class="fas fa-eye"></i></a> &nbsp; 
                                        
                                        @if($user->user_type == 6 || $user->user_type == 3 || $user->user_type == 1)
                                            <a href="{{url('purchase-order/bulk/invoice/list/'.$purchase_orders[$i]->id)}}"><i title="PO Invoices" class="fas fa-eye"></i></a> &nbsp;   
                                        @endif        
                                         <?php /* ?>
                                        @if($user->user_type == 3)
                                            <a href="{{url('purchase-order/bulk/edit/'.$purchase_orders[$i]->id)}}"><i title="Edit Order" class="fas fa-edit"></i> </a> 
                                        @endif    <?php */ ?>
                                    </td>
                                </tr>
                            @endfor
