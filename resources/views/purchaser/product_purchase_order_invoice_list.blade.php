...
@if(empty($error_message))
    <?php $breadCrumbArray = array(array('name'=>'Dashboard','link'=>'dashboard'),array('name'=>'Purchase Orders','link'=>'purchase-order/product/list'),array('name'=>'Purchase Order Invoice list')); ?>
            ...
            <form method="get">
                @if(in_array($user->user_type,[1,6]) || $is_fake_inventory_user)
                    <div class="row justify-content-end" >
                        <div class="col-md-2" ><button type="button" id="pos_add_invoice_submit" name="pos_add_invoice_submit" class="btn btn-dialog" value="Submit" onclick="displayAddPOInvoice();">Add Invoice</button>&nbsp;&nbsp;</div>
                    </div>
                @endif
            </form>
            ...
            <div id="productsContainer">
                <div class="table-responsive table-filter">
                    <table class="table table-striped admin-table" cellspacing="0">
                        <thead><tr class="header-tr"><th>SNo</th><th>ID</th><th>Invoice No</th><th>Invoice Date</th><th>Inventory Count</th><th>Vehicle No</th><th>Containers Count</th><th>GRN</th><th>QC</th><th>Created By</th><th>Created On</th><th>Action</th></tr></thead>
                        <tbody>
                            @for($i=0;$i<count($po_invoices);$i++)
                                <tr>
                                    ...
                                    <td>{{date('d M Y',strtotime($po_invoices[$i]->created_at))}}</td>
                                    <td>
                                        <a href="javascript:;" onclick="displayPOInvoiceDetails({{$po_invoices[$i]->id}});"><i title="Invoice Details" class="fas fa-eye"></i> &nbsp; 
                                        <?php $import_icon = empty($po_invoices[$i]->grn_id)?'fa-edit':'fa-eye'; ?>    
                                        @if(in_array($user->user_type,[1,6]) || $is_fake_inventory_user)    
                                            <a href="{{url('warehouse/sor/inventory/import/'.$po_invoices[$i]->id)}}"><i title="Import Inventory" class="fas {{$import_icon}}"></i> &nbsp; 
                                        @else        
                                            @if(!empty($po_invoices[$i]->grn_id))    
                                                <a href="{{url('warehouse/sor/inventory/import/'.$po_invoices[$i]->id)}}"><i title="Import Inventory" class="fas fa-eye"></i> &nbsp; 
                                            @endif    
                                        @endif    
                                        
                                        <?php $qc_icon = empty($po_invoices[$i]->qc_id)?'fa-edit':'fa-eye'; ?> 
                                        @if(in_array($user->user_type,[1,6]) || $is_fake_inventory_user)    
                                            <a href="{{url('warehouse/sor/inventory/qc/'.$po_invoices[$i]->id)}}"><i title="Inventory QC" class="fas {{$qc_icon}}"></i>    
                                        @else
                                            @if(!empty($po_invoices[$i]->qc_id))    
                                                <a href="{{url('warehouse/sor/inventory/qc/'.$po_invoices[$i]->id)}}"><i title="Inventory QC" class="fas fa-eye"></i>    
                                            @endif    
                                        @endif    
                                        
                                        @if(in_array($user->user_type,[1,6]) && empty($po_invoices[$i]->grn_id) && empty($po_invoices[$i]->qc_id) )   
                                            &nbsp;
            ...
            <div class="modal-content">
                ...
                <form class="" name="addVehicleDetailsForm" id="addVehicleDetailsForm" type="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-footer center-footer">
                        ...
                        <button type="button" id ="add_vehicle_details_confirm_submit" name="add_vehicle_details_confirm_submit" class="btn btn-dialog" onclick="submitAddPOInvoice();">Submit</button>
                        <input type="hidden" name="po_id" id="po_id" value="{{$po_data->id}}">
                    </div>
                </form>
            </div>
                ...
                <form class="" name="vehicleDetailForm" id="vehicleDetailForm" type="POST" >
                    ...
                    @if($user->user_type == 6)
                        <div class="modal-footer center-footer">
                            ...
                            <button type="button" id ="update_po_invoice_detail_submit" name="update_po_invoice_detail_submit" class="btn btn-dialog" onclick="submitUpdatePOInvoiceDetails();">Submit</button>
...
@section('scripts')
<script src="{{ asset('js/warehouse_po.js?v=1.135') }}" ></script>
