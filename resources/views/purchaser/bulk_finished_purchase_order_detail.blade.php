...
@if(empty($error_message))
    <?php $breadCrumbArray = array(array('name'=>'Dashboard','link'=>'dashboard'),array('name'=>'Stitching Purchase Orders','link'=>'purchase-order/bulk/finished/list'),array('name'=>'Stitching Purchase Order Detail')); ?>
    <?php echo CommonHelper::pageSubHeader($breadCrumbArray,'SOR Purchase Order Detail'); ?>
        ...
        <div class="container-fluid" >
            <form id="order_detail_form" name="order_detail_form">
                <div class="form-row">
                    ...
                    @if($user->user_type == 3 || $user->user_type == 12 || $user->user_type == 13)
                        <div class="form-group col-md-2">
                            <label for="Category">Purchase Order</label>						
                            <a class="btn btn-dialog" href="{{url('purchase-order/bulk/finished/invoice/'.$purchase_order_data->id)}}">Download PO</a>
                        </div>
                    @endif
                    ...
                    @if($user->user_type == 3 || $user->user_type == 6)
                        <div class="form-group col-md-2">
                            ...
                            <a class="btn btn-dialog" href="{{url('purchase-order/bulk/finished/detail/'.$purchase_order_data->id.'?action=ean_csv')}}">Download CSV</a>    
                        </div>
                    @endif
                
                    <div class="form-group col-md-2">
                        <label for="Category">SKU CSV</label>						
                        <a class="btn btn-dialog" href="{{url('purchase-order/bulk/finished/detail/'.$purchase_order_data->id.'?action=sku_csv')}}">Download SKU CSV</a>    
                    </div>
                </div>
            </form> 
            <hr/>
