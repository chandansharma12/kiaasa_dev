...
<?php $currency = CommonHelper::getCurrency(); ?>
    ...
    <table class="table-1" >
        <tr>
            <td>SNo.</td><td width="15%">Item Code</td><td>Fabric</td>
            <td >Color</td><td>Rate</td><td>Qty</td><td>Taxable<br>Value</td>
        ...
        @for($i=0;$i<count($qc_return_items);$i++)    
            <?php $product_data = $qc_return_items[$i]; ?>
            <?php $unit_code = $product_data->unit_code; ?>
            <tr>
                <td>{{$i+1}}</td>
                <td>{{$product_data->sku}}</td>
                <td>{{$product_data->fabric_name}}</td>
                <td>{{$product_data->color_name}}</td>
                <td>{{$product_data->rate}}</td>
                <td>{{$product_data->quantity}} {{$product_data->unit_code}}</td>
                <td>{{$taxable_val = round($product_data->rate*$product_data->quantity,2)}}</td>
                
                @if($gst_name == 's_gst')
                    <td>{{$product_data->gst_percent/2 }}%</td>
                    <td style="text-align: left;">{{round($taxable_val*($product_data->gst_percent*0.5/100),2)}}</td>
                    <td>{{$product_data->gst_percent/2 }}%</td>
                    <td style="text-align: left;">{{round($taxable_val*($product_data->gst_percent*0.5/100),2)}} </td>
                    <td colspan="2"></td>
                @endif
                ...
                @if($gst_name == 'i_gst')
                    ...
                    <td>{{round($taxable_val*($product_data->gst_percent/100),2)}}</td>
                @endif
        ...
        <tr>
            ...
            <td class="border-top border-bottom">{{$total_qty}} {{(isset($unit_code)?$unit_code:'')}}</td>
