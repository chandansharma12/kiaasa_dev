...
<?php $cancel_str = ($demand_data->demand_status == 'cancelled')?'(Cancelled)':'';  ?>
    ...
    <table class="table-1">
        ...
        <tr>
            <td  width="50%" valign="top" >
                ...
                <b>{{strtoupper($vendor_data->name)}}</b><br/>
                <span class="text-2">Address : {{$vendor_data->address}}<br/>{{$vendor_data->city}} - {{$vendor_data->postal_code}}<br/>
                Ph.No. : {{$vendor_data->phone}}<br/>
                GSTIN NO.: {{$vendor_data->gst_no}}</span>
            </td>
    ...
    <table class="table-1 border-top border-bottom" >
        <tr><td width="50%">Invoice No:&nbsp;{{$demand_data->invoice_no}}<span class="text-3"> </span></td><td width="50%">Invoice Date:&nbsp;<span class="text-3">{{date('d/m/Y H:i',strtotime($demand_data->created_at))}} </span></td></tr>
    </table>    
