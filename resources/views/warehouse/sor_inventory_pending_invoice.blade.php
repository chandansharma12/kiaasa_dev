...
<?php $cancel_str = ($debit_note_data->debit_note_status == 'cancelled')?'(Cancelled)':'';  ?>
    ...
    @if(strtolower($invoice_type) == 'debit_note')
        <table class="table-1">
            <tr><td></td><td class="heading-1"><b>Debit Note  {{$cancel_str}}</b></td></tr>
    ...
    @if(strtolower($invoice_type) == 'credit_note')
        <table class="table-1">
            <tr><td></td><td class="heading-1"><b>Credit Note  {{$cancel_str}}</b></td></tr>
            <tr><td colspan="2"><div class="separator-10"></div></td></tr>
            <tr>
                <td width="50%" valign="top" class="text-1">
                    <b class="text-1">Supplier: {{$po_data->name}}</b><br/>
    ...
    <table class="table-1 border-top border-bottom" >
        <tr>
            <td>
            @if(strtolower($invoice_type) == 'debit_note')
                Invoice No:&nbsp;<span class="text-2">{{$debit_note_data->debit_note_no}}  </span> &nbsp; Date: {{date('d-m-Y',strtotime($debit_note_data->created_at))}}
            @endif
            @if(strtolower($invoice_type) == 'credit_note')
                Invoice No:&nbsp;<span class="text-2">{{$debit_note_data->credit_note_no}} </span> &nbsp; Date: {{date('d-m-Y',strtotime($debit_note_data->created_at))}}
            @endif
            &nbsp;&nbsp; Purchase Order Invoice No:&nbsp;<span class="text-2">{{$po_data->invoice_no}} </span>
            &nbsp;&nbsp; Invoice Date:&nbsp;<span class="text-2">{{date('d-m-Y',strtotime($po_data->invoice_date))}} </span>
            &nbsp;&nbsp; GRN No:&nbsp;<span class="text-2">{{$po_data->grn_no}} </span>
            </td>
        </tr>
    </table>    
    ...
    <table class="table-1" >
        ...
        @foreach($sku_list as $sku=>$product_data)    
            <tr>
                <td>{{$i+1}}</td>
        ...
        <tr>
            <td colspan="7" align="left">
                Remarks: {{$debit_note_data->comments}}
            </td>
