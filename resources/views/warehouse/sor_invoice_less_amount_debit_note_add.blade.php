...
@if(empty($error_message))
    <?php $breadCrumbArray = array(array('name'=>'Dashboard','link'=>'warehouse/dashboard'),array('name'=>'Purchase Orders','link'=>'purchase-order/product/list'),array('name'=>'Purchase Order Invoice List','link'=>'purchase-order/product/invoice/list/'.$po_data->id),array('name'=>'Create SOR Purchase Order Less Amount Invoice Debit Note ')); ?>
    <?php echo CommonHelper::pageSubHeader($breadCrumbArray,'Create SOR Purchase Order Less Amount Invoice Debit Note' ); ?>
                            ...
                            <table class="table table-striped admin-table" cellspacing="0">
                                <thead>
                                    <tr class="header-tr">
                                        ...
                                        <th>Invoice Cost</th><th>Invoice Amount</th><th>Debit Note Amount</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    @for($i=0;$i<count($sku_list);$i++)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="numeric-text" name="invoice_cost_{{($sku_list[$i]->id)}}" id="invoice_cost_{{($sku_list[$i]->id)}}" onkeyup="updateSorLessDebitNoteInvoiceAmount(this,{{($sku_list[$i]->id)}},{{$sku_list[$i]->inv_count}},{{$po_amount}});" value="{{isset($sku_data['item_invoice_cost'])?$sku_data['item_invoice_cost']:''}}">
                                ...
                                <tfoot>
                                    <tr>
                                        <td colspan="12" align="center">
                                            ...
                                            <button type="button" id="add_less_amount_debit_note_submit" class="btn btn-dialog" onclick="addPOInvoiceLessAmountDebitNote();">Submit</button> &nbsp;
                                            <button type="button" data-dismiss="modal" class="btn btn-secondary" id="add_less_amount_debit_note_cancel">Cancel</button>
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
